    ...
    <div>
      <Table
        ...
        columns={normalizeColumns([
          ...
          {
            key: 'tags', title: t('标签'), render: tags => (
              <div style={{whiteSpace: 'normal'}}>
                {(tags || []).map((v, i) => <Tag onClick={() => setCurrentTag(v)} key={i}>{v}</Tag>)}
              </div>
            )
          },
        ])}
        ...
        title={() => {
          return (
            <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
              <h2><Trans>标签</Trans></h2>
              <div>
                {currentTag && <Tag closable onClose={() => setCurrentTag(null)}>{currentTag}</Tag>}
                <Button.Group>
