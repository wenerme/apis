...
const KongAdminSummary: React.FC = () => {
  ...
  const statusItems: Array<DataDescriptionProps<KongNodeStatus>> = [
    {label: '数据库可访问', name: 'database.reachable', span: 2, render: ({value}) => <Checkbox checked={Boolean(value)} />},
    {
      label: 'LUA Workers', span: 2, render({record}) {
        return record.memory.workers_lua_vms.length
      }
    },
    {
      label: 'LUA Worker 内存', span: 4, render({record}) {
        return formatBytes(record.memory.workers_lua_vms.map(v => parseBytes(v.http_allocated_gc)).reduce((a, b) => a + b, 0))
      }
    },
    {
      label: 'LUA 共享字典内存', span: 4, render({record}) {
          ...
          <div>
            ...
            <Card title={t('插件列表')}>
              <div style={{display: 'grid', gridAutoFlow: 'row', rowGap: 12}}>
                <Card title={t('可用插件')}>
                  <div>
                    {Object.keys(information?.plugins?.available_on_server ?? {}).map(v => <Tag key={v}
                                                                                                children={v} />)}
                  </div>
                </Card>
                ...
                <Card title={t('集群启用插件')}>
                  <div>
                    {Object.keys(information?.plugins?.enabled_in_cluster ?? {}).map(v => <Tag key={v} children={v} />)}
