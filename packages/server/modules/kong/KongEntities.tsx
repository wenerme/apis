...
export const KongEntities: Record<string, { attributes, fields?, columns? } & any> = {
  ...
  ConsumerOAuth2: {
    ...
    attributes: [
      ...
      {key: 'redirect_uris', label: 'Redirect Uris', widget: 'select', widgetProps: {mode: 'tags'}},
    ],
  },
  ConsumerKeyAuth: {
...
export function buildEntity(entity) {
  ...
  // table
  entity.columns = attributes.filter(({view}) => typeof view === 'boolean' ? view : true).map(({key, label, render, dataIndex, title, width}) => {
    return {key, render, dataIndex: dataIndex || key.split('.'), title: title || label, width: width || 100}
  });
