...
const OIDs = {
  ...
  '2.5.29.54': {label: 'X.509 version 3 certificate extension Inhibit Any-policy'},
  ...
  '1.2.840.10045.4.3.2': {label: 'Elliptic Curve Digital Signature Algorithm (DSA) coupled with the Secure Hash Algorithm 256 (SHA256) algorithm'},
    ...
    <div>
      <div>
        ...
        <Descriptions column={1} size="small">
          {
            cert.subject.typesAndValues.map(v => [v.type, v.value.valueBlock.value])
              .map(([k, v]) => (
        ...
        <Descriptions column={1} size="small">
          {
            cert.issuer.typesAndValues.map(v => [v.type, v.value.valueBlock.value])
              .map(([k, v]) => (
                <Descriptions.Item key={k} label={oidOf(k)}>
                  {stringOf(v)}
                </Descriptions.Item>
        ...
        <Descriptions column={1} size="small">
          <Descriptions.Item label="Algorithm">
            {oidOf(cert.subjectPublicKeyInfo.algorithm.algorithmId)} ({cert.subjectPublicKeyInfo.algorithm.algorithmId + ''})
          </Descriptions.Item>
          <Descriptions.Item label="Parameter">
        ...
        <h3>Extensions</h3>
        {
          (cert.extensions ?? [])
            .map((e) => (
              <div>
                <h4>Extension {oidOf(e.extnID)} ({e.extnID})</h4>
                {
                  <Descriptions column={1} size="small">
