...
export async function findByPhoneNumber(
  ...
  const entities = await em.getRepository(PhoneDataIndexEntity).find({
    where: {
  ...
  const record = await em.getRepository(PhoneDataRecordEntity).findOne({ offset: index.recordOffset });
  return { index, record };
}
...
async function saveChunks<T>(
  ...
  for (const chunk of chunks) {
    await em.createQueryBuilder().insert().into(entityTarget).values(chunk).execute();
  }
}
