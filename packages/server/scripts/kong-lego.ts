...
async function main() {
  ...
  const {data: {data: certs}} = await axios.get(`${API}/certificates?tags=lego`);

  certs.forEach(v => {
    v.domains = v.tags.filter(v => !v.startsWith('lego'))
      .map(v => {
        // normalize
        return v.replace(/^_[.]/, '*.')
      });
    v.legoOptions = Object.fromEntries(v.tags
  ...
  for (const cert of Object.values(certDataByDomain)) {
    ...
    if (!entity) {
      ...
      try {
        const {id} = await resultOf(axios.post(`${API}/certificates`, {
          cert: crt,
          key,
          tags: ['lego', domain]
        }));
      ...
      try {
        await resultOf(axios.post(`${API}/certificates/${entity.id}`, {
          cert: crt,
          key,
          tags: entity.tags
        }));
