    ...
    <div className="container">
      <div style={{margin: '0 12px'}}>
        ...
        <Descriptions column={4}>
          {desc.map(({label, name, render, ...props}) => (
            <Descriptions.Item
              label={label}
              key={label}
              {...props}
            >
              {render ? torrent?.[name] && render(torrent?.[name]) : torrent?.[name]}
            </Descriptions.Item>
          ))}
        </Descriptions>
        ...
        <Input.TextArea style={{minHeight: 200}} readOnly value={torrent?.announce?.join('\n') ?? ''} />
      </div>
      <div>
        <div>
