...
function buildExplicitNestRestMethod(name: string, client, { path = '', parentPath }) {
  ...
  return {
    ...
    [`update${n}`]: (parent, entity) =>
      resultOf(client.patch(`/${pp}/${idOrName(parent)}/${p}/${idOrName(entity)}`, entity)),
    [`delete${n}`]: (parent, entity) => resultOf(client.delete(`/${pp}/${idOrName(parent)}/${p}/${idOrName(entity)}`)),
    [`get${n}ByIdOrName`]: (parent, entity) =>
      resultOf(client.get(`/${pp}/${idOrName(parent)}/${p}/${idOrName(entity)}`)),
  };
}
