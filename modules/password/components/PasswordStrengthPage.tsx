...
const ZxcvbnDescription: React.FC<{ password, result? }> = ({password, result}) => {
  ...
  const crackTimes = [
    ...
    {label: '10,000 个每秒', speedTip: '离线破解, 较慢的算法, 多核', value: times.offline_slow_hashing_1e4_per_second,},
    ...
    <div>
      <Descriptions bordered>
        ...
        <Descriptions.Item label={'猜测次数'}>
          <div>{guesses}</div>
          <div>{guesses_log10.toFixed(6)} <small>LOG10</small></div>
        </Descriptions.Item>
...
function suggest(seed) {
  ...
  const generator = createPasswordGenerator({random, upper: false, symbol: false});
      ...
      <div>
        {suggests.map(v => (
          <Link key={v} href="/password/strength/[password]" as={`/password/strength/${v}.html`}>
            <a href={`/password/strength/${v}.html`} className="ant-btn ant-btn-link">{v}</a>
          </Link>
        ))}
      </div>
    </div>
